"""
Django settings for InstaFood backend.
"""
...
# ─── Database ──────────────────────────────────────────────────────────────────
# Uses PostgreSQL by default via DATABASE_URL env variable.
# Fallback to SQLite for local development (set SQLITE=true in .env).
if os.environ.get('SQLITE', 'false').lower() == 'true':
    DATABASES = {
        'default': {
...
REST_FRAMEWORK = {
    ...
    # Allow read-only endpoints without auth; require auth for mutations
